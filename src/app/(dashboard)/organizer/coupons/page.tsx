...
import { Tag, Plus, Trash2, Calendar, Percent, DollarSign, X, Check, Loader2, ArrowUpDown } from "lucide-react";
...
export default function VendorCouponsPage() {
  ...
  const fetchCouponsAndEvents = async () => {
    try {
      ...
      // Fetch events for scoping dropdown
      ...
      // Fetch coupons created by vendor
  ...
  const getCouponStatus = (coupon: Coupon) => {
    ...
    if (coupon.max_uses !== null && coupon.used_count >= coupon.max_uses) {
      return { label: "Limit Reached", color: "bg-amber-500/10 text-amber-400 border-amber-500/25" };
    }
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold font-heading">My Coupons</h1>
            <p className="text-zinc-450">Create discount codes for your ticket tiers.</p>
          </div>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="glass-card p-12 flex flex-col items-center text-center space-y-4 border-dashed border-white/5 bg-transparent">
          ...
          <div>
            ...
            <p className="text-zinc-500 text-sm max-w-xs mx-auto mt-1">Create coupons to reward buyers and boost ticket sales.</p>
          </div>
        </div>
      ) : (
                  ...
                  return (
                    <tr key={coupon.id} className="hover:bg-white/2 transition-colors">
                      ...
                      <td className="p-4 sm:p-5 text-zinc-300">
                        {coupon.events?.title ? (
                          <span className="text-indigo-400 font-medium line-clamp-1 max-w-[200px]" title={coupon.events.title}>
                            {coupon.events.title}
                          </span>
                        ) : (
                          <span className="text-zinc-500 font-bold uppercase text-xs tracking-wider">All my events</span>
                        )}
                      </td>
                      ...
                      <td className="p-4 sm:p-5 text-xs text-zinc-400 space-y-0.5">
                        ...
                        {coupon.valid_until ? (
                          ...
                          <p className="text-zinc-600">Never expires</p>
                        )}
                      </td>
                      <td className="p-4 sm:p-5">
          ...
          <div className="bg-zinc-900 border border-white/10 rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-bold font-heading text-white">Create Coupon</h3>
              </div>
              <button
            ...
            <form onSubmit={handleCreateCoupon} className="space-y-4 text-zinc-300">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-xs font-bold text-zinc-400">Scope Event</label>
                  <select
                    value={eventId}
