...
export default async function Home() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#FAF9F6] text-zinc-900 overflow-hidden">
      ...
      {/* 2. Hero Section */}
      <main className="flex-1 relative pt-32 pb-16 px-6 md:px-12 flex flex-col items-center">
        {/* Decorative background glow */}
        <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[500px] bg-indigo-500/5 blur-[120px] rounded-full pointer-events-none -z-10" />

        <div className="relative z-10 w-full max-w-4xl flex flex-col items-center text-center mt-8 mb-20">
          <span className="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full bg-indigo-50 border border-indigo-100 text-xs font-bold text-indigo-600 mb-6 uppercase tracking-wide">
            🎟️ Over 50,000+ tickets sold
          </span>
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold font-heading tracking-tight leading-tight mb-6 text-indigo-500">
            Experience the Best Events <br className="hidden sm:inline" /> in Nigeria
          </h1>
          <p className="text-base sm:text-lg text-zinc-500 max-w-2xl mb-10 leading-relaxed">
            Discover, book, and access premium concerts, conferences, local festivals, and shows across Nigeria. Secure checkout and instant QR ticket delivery.
          </p>
          
          {/* Hero Search Bar */}
          <div className="glass-card w-full max-w-3xl flex flex-col md:flex-row items-center p-2.5 gap-2.5 shadow-md border-[#E8EBE7]">
            <div className="flex-1 flex items-center gap-3 px-4 py-3.5 w-full border-b border-zinc-200/50 md:border-b-0 md:border-r md:border-zinc-200/50">
              <Search className="w-5 h-5 text-indigo-500/60" />
              <input 
                type="text" 
                placeholder="Search events, locations, categories..." 
                className="bg-transparent border-none outline-none w-full text-zinc-800 placeholder:text-zinc-400 text-sm font-medium"
              />
            </div>
            <div className="w-full md:w-48 flex items-center gap-3 px-4 py-3.5 border-b border-zinc-200/50 md:border-b-0">
              <MapPin className="w-5 h-5 text-indigo-500/60" />
              <input 
                type="text" 
            ...
            <button className="w-full md:w-auto px-8 py-3.5 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white font-bold transition-all flex items-center justify-center gap-2 shadow-lg shadow-amber-500/10 cursor-pointer">
              Search
            </button>
          </div>

          {/* Core Features bullets */}
          <div className="flex flex-wrap items-center justify-center gap-x-8 gap-y-3 mt-10 text-xs font-bold text-zinc-500 uppercase tracking-wider">
            <span className="flex items-center gap-1.5"><CheckCircle2 className="w-4 h-4 text-indigo-500" /> 50,000+ happy fans</span>
            <span className="flex items-center gap-1.5"><ShieldCheck className="w-4 h-4 text-indigo-500" /> Secure Paystack checkout</span>
            <span className="flex items-center gap-1.5"><Zap className="w-4 h-4 text-indigo-500" /> Instant QR tickets</span>
          </div>
        </div>

        {/* 3. Browse by Category */}
        <div className="w-full max-w-6xl mb-24 relative z-10">
